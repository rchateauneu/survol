...
def AddSymbolInClass( symbol, file, prop ):
...
def AddKnown(symbol, file, type):
	symbolNode = AddSymbolInClass( symbol, file, pc.property_symbol_defined )
	grph.add( ( symbolNode, pc.property_symbol_type, rdflib.Literal(type) ) )

def AddUnknown(symbol):
	symbolNode = AddSymbolInClass( symbol, "*", pc.property_symbol_undefined )
