...
def TcpDumpEngine(sharedTupleQueue,entityId):
	print("pid="+str(os.getpid()) + " " + "Filling queue with junk")
	for i in range(0,100000):
		...
		print("pid="+str(os.getpid()) + " " + "Pushing junk")
		sharedTupleQueue.put( my_tuple )
		print("pid="+str(os.getpid()) + " " + "Pushed junk")
	print("pid="+str(os.getpid()) + " " + "Filled queue with junk")

	# Option -n so no conversion of addresses and port numbers.
	tcpdump_cmd = "sudo tcpdump -n"
	print("pid="+str(os.getpid()) + " " + "TCPcommand=" + tcpdump_cmd)
