...
def DirToMenu(grph,parentNode,curr_dir,relative_dir,depthCall = 1):
	...
	for dir in dirs:
		# sys.stderr.write(txtMargin + "dir=%s\n"%(dir))
		...
		if not currDirNode:
			sys.stderr.write( txtMargin + "currDirNode NONE: argDir=%s dir=%s\n"%(argDir,dir))
			continue
		...
		# sys.stderr.write( txtMargin + "Checked ontology of %s: ontoKeys=%s\n"%(sub_entity_class,str(ontoKeys)))
	...
	for fil in files:
		...
		# sys.stderr.write(txtMargin + "DirToMenu encodedEntityId=%s\n" % encodedEntityId)
		if is_host_remote:
