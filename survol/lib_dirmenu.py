...
def _add_cim_links(callback_grph_add, parent_node, entity_type, entity_id, gen_obj):
    ...
    try:
        ...
        # Typically with pypy where wmi cannot be installed. This error is not a blocker.
        logging.warning("_add_cim_links entity_type=%s entity_id=%s: Caught %s",
...
# TODO: For the moment, it returns two types of menus, with or without seeAlso.
# In the near future, only the version with seeAlso will be used..
def recursive_walk_on_scripts(
        callback_grph_add, parent_node, entity_type, entity_id, entity_host, flag_show_all,
        with_see_also=False):
    """
