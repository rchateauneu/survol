...
#
# Namescape and class are considered similar.
...
class ElfSym:
    ...
    def __str__(self):
        return 't=%-7s b=%-6s v=%-7s FU=%s A=%s' % (
            self.m_type, self.m_bind, self.m_vis, self.m_short_nam, str(self.m_args))


def indexStartsWith(the_str, prefix):
    l = len(prefix)
    if the_str.startswith(prefix):
        return the_str[l:]
    else:
        return None 
