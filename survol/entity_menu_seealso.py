...
# pc_property_information = pc.property_information
pc_property_information = rdflib.namespace.RDFS.label

# pc_property_script = pc.property_script
DCT = rdflib.Namespace("http://purl.org/dc/terms/")
pc_property_script = rdflib.URIRef(DCT["hasPart"])
...
def _add_wmi_links(callback_grph_add, parent_node, entity_type, entity_id, gen_obj):
    ...
    for attribute_name in attributes_list:
        ...
        if wmi_associators_node is None:
            ...
            callback_grph_add((parent_node, pc_property_script, wmi_associators_node), depth_call)
        ...
        callback_grph_add((wmi_associators_node, pc_property_script, script_node), depth_call + 1)
        script_label, _, _ = lib_naming.ParseEntityUri(url_path)
        callback_grph_add((script_node, pc_property_information, lib_util.NodeLiteral(script_label)), depth_call + 1)
        # TODO: GuessDisplayMode should use the header.
        url_seeAlso = url_path + "&mode=rdf"
        callback_grph_add((script_node, rdflib.namespace.RDFS["seeAlso"], rdflib.term.URIRef(url_seeAlso)), depth_call + 1)
    ...
    def recursive_walk_aux(a_parent_node, grand_parent_node, curr_dir, relative_dir, depth_call=1):
        ...
        if not entity_host:
            ...
            if err_dir_node:
                if flag_show_all:
                    ...
                    callback_grph_add((grand_parent_node, pc_property_script, curr_dir_node), depth_call)
        ...
        for dir in dirs:
            ...
            if something_added:
                ...
                callback_grph_add((a_parent_node, pc_property_script, curr_dir_node), depth_call)
        ...
        for fil in files:
            ...
            callback_grph_add((a_parent_node, pc_property_script, rdf_node), depth_call)
            ...
            callback_grph_add((rdf_node, pc_property_information, nod_modu), depth_call)
            #         # TODO: GuessDisplayMode should use the header.
            #         url_seeAlso = url_path + "&mode=rdf"
            # rdflib.term.URIRef(url_seeAlso)
            url_seeAlso = rdf_node + "&mode=rdf"
            callback_grph_add((rdf_node, rdflib.namespace.RDFS["seeAlso"], rdflib.term.URIRef(url_seeAlso)),
                              depth_call)
