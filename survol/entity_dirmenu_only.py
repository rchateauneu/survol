...
def recursive_walk_on_scripts(callback_grph_add, parent_node, entity_type, entity_id, entity_host, flag_show_all):
    ...
    def directory_usability_error_node(relative_dir, depth_call):
        ...
        try:
            ...
            #_dir_menu_report(depth_call, "entity_class=%s\n"%(entity_class))
    ...
    def recursive_walk_aux(a_parent_node, grand_parent_node, curr_dir, relative_dir, depth_call=1):
        ...
        #_dir_menu_report(depth_call, "curr_dir=%s relative_dir=%s\n"%(curr_dir,relative_dir))
        ...
        for dir in dirs:
            #_dir_menu_report(depth_call, "dir=%s\n"%(dir))
            ...
            if not curr_dir_node:
                #_dir_menu_report(depth_call, "curr_dir_node NONE: arg_dir=%s dir=%s\n"%(arg_dir,dir))
                continue
            ...
            #_dir_menu_report(depth_call, "Checked ontology of %s: onto_keys=%s\n"%(sub_entity_class,str(onto_keys)))
        ...
        for fil in files:
            ...
            #_dir_menu_report(depth_call, "DirToMenu encoded_entity_id=%s\n" % encoded_entity_id)
    ...
    if entity_host:
        gen_obj = lib_common.RemoteBox(entity_host)
    else:
        gen_obj = lib_common.gUriGen
