...
def Main():
	...
	if iEjList:
		for iEjOne in iEjList:
			...
			grph.add( ( symNod, lib_util.MakeProp("vaddr"), lib_util.NodeLiteral(iE_vaddr) ) )
			grph.add( ( symNod, lib_util.MakeProp("paddr"), lib_util.NodeLiteral(iE_paddr) ) )
			grph.add( ( symNod, lib_util.MakeProp("type"), lib_util.NodeLiteral(iE_type) ) )
			grph.add( ( symNod, lib_util.MakeProp("bind"), lib_util.NodeLiteral(iE_bind) ) )
