...
def GetInfoConnections(grph,sessionId,nodeSession,cnxn):
	...
	for rowConnections in cursorConnections.execute(qryConnections):
		grph.add( (nodeSession, lib_util.MakeProp("Net transport"), lib_util.NodeLiteral(rowConnections.net_transport) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Protocol type"), lib_util.NodeLiteral(rowConnections.protocol_type) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Auth scheme"), lib_util.NodeLiteral(rowConnections.auth_scheme) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Connect time"), lib_util.NodeLiteral(rowConnections.connect_time) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Last read"), lib_util.NodeLiteral(rowConnections.last_read) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Last write"), lib_util.NodeLiteral(rowConnections.last_write) ) )
...
def GetInfoSessions(grph,sessionId,nodeSession,cnxn):
	...
	for rowSessions in cursorSessions.execute(qrySessions):
		grph.add( (nodeSession, lib_util.MakeProp("Client interface"), lib_util.NodeLiteral(rowSessions.client_interface_name) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Login"), lib_util.NodeLiteral(rowSessions.login_name) ) )
		if rowSessions.program_name:
			grph.add( (nodeSession, lib_util.MakeProp("Program"), lib_util.NodeLiteral(rowSessions.program_name) ) )

		grph.add( (nodeSession, lib_util.MakeProp("Login time"), lib_util.NodeLiteral(rowSessions.login_time) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Last request start"), lib_util.NodeLiteral(rowSessions.last_request_start_time) ) )
		if rowSessions.last_request_end_time:
			grph.add( (nodeSession, lib_util.MakeProp("Last request end"), lib_util.NodeLiteral(rowSessions.last_request_end_time) ) )
...
def GetInfoRequests(grph,sessionId,nodeSession,cnxn,dsnNam):
	...
	for rowRequests in cursorRequests.execute(qryRequests):
		grph.add( (nodeSession, lib_util.MakeProp("Status"), lib_util.NodeLiteral(rowRequests.status) ) )
		grph.add( (nodeSession, lib_util.MakeProp("Command"), lib_util.NodeLiteral(rowRequests.command) ) )
...
def Main():
	...
	try:
		...
		grph.add( (nodeSession, lib_util.MakeProp("Session id"), lib_util.NodeLiteral(sessionId) ) )
